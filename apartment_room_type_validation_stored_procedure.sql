CREATE DEFINER=`dbmasteruser`@`%` PROCEDURE `apartment_room_type_validation_stored_procedure`(
...
BEGIN
    ...
    Declare apartmentRoomTypeIdParam int;
	...
	WHILE x <= recordNumber DO
        ...
        IF (productNameValue = '') THEN
			IF ((select COUNT(*) from apartment_room_type where TRIM(apartment_room_type_name) = TRIM(apartmentRoomTypeNameValue)) != 0) THEN
				SET invalidRows = CONCAT(invalidRows, 'Apartment Room Type Name is already exist row(s): ', x, '; ');
			END IF;
		ELSE
			IF ((select COUNT(*) from apartment_room_type where TRIM(apartment_room_type_name) = TRIM(apartmentRoomTypeNameValue)) = 0) THEN
				SET invalidRows = CONCAT(invalidRows, 'Apartment Room Type Name does not exist row(s): ', x, '; ');
			END IF;
        END IF;
